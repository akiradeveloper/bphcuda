...
public:
  ...
  real6 operator()(const real7 &in) const {
    real3 p = real3(get<0>(in), get<1>(in), get<2>(in));
    real3 c = real3(get<3>(in), get<4>(in), get<5>(in));
    real m = get<6>(in);
    real3 force = _f(in);
    real3 a = force / m;
...
public:
  ...
  real6 operator()(const real7 &in) const {
     ...
     */
    real3 h_c = real3(get<3>(h_state), get<4>(h_state), get<5>(h_state));
